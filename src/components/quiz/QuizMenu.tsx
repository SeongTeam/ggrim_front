...
const ShowDescription = ({ quiz, isShow, setShow }: ShowDescriptionProps) => {
	...
	const handleClickTag = (name: string) => {
		const url = SEARCH_LOGIC_ROUTE.SEARCH_PAINTING(`tag:${name}`);
		router.push(url);
	};

	const handleClickStyle = (name: string) => {
		const url = SEARCH_LOGIC_ROUTE.SEARCH_PAINTING(`style:${name}`);
		router.push(url);
	};

	const handleClickArtist = (name: string) => {
		const url = SEARCH_LOGIC_ROUTE.SEARCH_PAINTING(`artist:${name}`);
		router.push(url);
	};
